...
This is a separate integration path from bandofy.api.authorize_mac_on_omada
...
Run as a long-lived process, e.g. from the Procfile:
    ...
    radius: bench --site <site> execute bandofy.radius_server.start
"""
	...
	def HandleAuthPacket(self, pkt):
		...
		if self._enabled:
			try:
				...
				frappe.log_error(
					title="Bandofy RADIUS: Auth Packet Error", message=frappe.get_traceback()
				)
...
def validate_session(username, password):
	...
	``password`` is the one-time token bandofy.api generates once a
...
def start(bind_ip=None, auth_port=None, acct_port=None):
	"""Entrypoint for ``bench execute bandofy.radius_server.start``."""
	settings = frappe.get_single("Hotspot RADIUS Settings")
	...
	frappe.logger("bandofy").info(
		f"Bandofy RADIUS server listening on {resolved_bind_ip}:{resolved_auth_port} "
