...
frappe.pages["network-dashboard"].on_page_load = function (wrapper) {
	...
	new foh_ms.NetworkDashboard(page);
};

frappe.provide("foh_ms");

foh_ms.NetworkDashboard = class NetworkDashboard {
	constructor(page) {
		this.page = page;
	...
	load() {
		...
		frappe.call({
			method: "foh_ms.foh_ms.page.network_dashboard.network_dashboard.get_dashboard_data",
			callback: (r) => {
